  ...
  return (
    <footer dir="ltr" className="mt-16 pt-6 pb-6 text-center text-xs" style={{ borderTop: "1px solid var(--border)", color: "var(--text-muted)" }}>
      <div className="flex flex-wrap justify-center gap-3">
        ...
        <span style={{ color: "var(--border)" }}>·</span>
        <Link href="/history" className="hover:opacity-80 transition">{isAr ? "سجل التنبيهات" : "History"}</Link>
        <span style={{ color: "var(--border)" }}>·</span>
        <Link href="/stats" className="hover:opacity-80 transition">{isAr ? "إحصائيات" : "Stats"}</Link>
        <span style={{ color: "var(--border)" }}>·</span>
        <Link href="/about" className="hover:opacity-80 transition">{isAr ? "عن البيان" : "About"}</Link>
        <span style={{ color: "var(--border)" }}>·</span>
